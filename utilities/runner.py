...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-p", "--print-stdout", required=False, action="store_true", help="Print stdout")
    parser.add_argument("-j", "--json", required=False, action="store_true", help="Create json report file")
    parser.add_argument("-a", "--allure", required=False, action="store_true", help="Run Allure report")
    parser.add_argument("--behave-options", required=False, type=str, help="Extra behave options")
...
def prepare_command_from_args() -> str:
    ...
    cmd = cmd + (f"-f allure_behave.formatter:AllureFormatter -o {allure_dir} " if arg.allure else "")
...
if __name__ == "__main__":
    ...
    if parse_args().allure:
        subprocess.run(f"allure serve {get_allure_report_dir()}", shell=True, cwd=PROJECT_DIR)
